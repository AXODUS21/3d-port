...
const Testimonials = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add("(min-width: 768px)", () => {
        // Horizontal scroll animation linked to vertical scroll
        // The section is tall (300vh), allowing us to scroll "through" it
        gsap.to(track, {
  ...
  return (
    <section 
      ref={sectionRef} 
      id="testimonials" 
      className="relative bg-zinc-950 h-[300vh] lg:h-[400vh]" // Tall section to allow scrolling
    >
      {/* Sticky Viewport - Solid Background */}
      <div className="sticky top-0 h-screen w-full flex flex-col overflow-hidden bg-zinc-950 z-30">
        ...
        {/* Section Header - Relative layout to prevent overlap */}
        ...
        {/* The Track - Takes remaining space */}
        <div 
          ref={trackRef}
                  ...
                  <div className="relative w-full h-full overflow-hidden">
                    <video
                      ref={(el) => {
                         if (el) {
                           ...
                           el.muted = mutedVideos.has(item.id)
                         }
                      }}
                      src={item.videoUrl}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                       <div className="w-16 h-16 rounded-full border border-white/20 flex items-center justify-center backdrop-blur-md">
                          {playingVideo === item.id ? <Pause className="text-white fill-white" size={20} /> : <Play className="text-white fill-white" size={20} />}
                       </div>
                    </div>

                    {/* Controls */}
                    <div className="absolute top-4 right-4 flex gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                       <button onClick={(e) => togglePlay(item.id, e)} className="p-2 hover:bg-white/10 rounded-full transition"><Play size={16} className="text-white" /></button>
                       <button onClick={(e) => toggleMute(item.id, e)} className="p-2 hover:bg-white/10 rounded-full transition">{mutedVideos.has(item.id) ? <VolumeX size={16} className="text-white"/> : <Volume2 size={16} className="text-white"/>}</button>
                       <button onClick={() => openModal(item)} className="p-2 hover:bg-white/10 rounded-full transition"><Maximize2 size={16} className="text-white"/></button>
                    </div>
